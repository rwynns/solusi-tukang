                    ...
                    <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                            <div class="sm:col-span-2">
                                    ...
                                    <span class="inline-block h-24 w-24 rounded-full overflow-hidden bg-gray-100"
                                        ...
                                        @if ($user->tukangProfile && $user->tukangProfile->profile_photo)
                                            <img src="{{ asset('storage/' . $user->tukangProfile->profile_photo) }}"
                                                ...
                                                id="current_image">
                                        @else
                                            <div class="h-full w-full bg-[#F3BD2B] flex items-center justify-center text-2xl text-white font-bold"
                                                id="default_avatar">
                                ...
                                <p class="mt-2 text-xs text-gray-500 font-roboto">Format: JPG, PNG, JPEG. Maks: 2MB.</p>
                            </div>
                            ...
                            <div class="col-span-1">
                                <label for="location_id" class="block mb-1 text-sm font-medium font-poppins">Lokasi
                                    Layanan</label>
                                <select name="location_id" id="location_id"
                                    ...
                                    @foreach ($locations as $location)
                                        <option value="{{ $location->id }}"
                                            {{ old('location_id', $user->tukangProfile ? $user->tukangProfile->location_id : '') == $location->id ? 'selected' : '' }}>
                                            {{ $location->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="bio" id="bio" rows="4"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-500 font-roboto">{{ old('bio', $user->tukangProfile ? $user->tukangProfile->bio : '') }}</textarea>
                                <p class="mt-1 text-xs text-gray-500 font-roboto">Ceritakan tentang pengalaman, keahlian,
                                    dan layanan yang Anda tawarkan</p>
                            </div>
                        </div>
