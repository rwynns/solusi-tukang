    ...
    <div class="bg-white shadow">
        <div class="px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold font-poppins text-gray-900 text-shadow-md">Edit Sub Jasa</h1>
        ...
        <div class="max-w-5xl mx-auto">
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    <form action="{{ route('sub-jasa.update', $subJasa) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                            <div class="sm:col-span-2">
                                <label for="jasa_id" class="block mb-1 text-sm font-medium font-poppins">Jasa Utama</label>
                                <select name="jasa_id" id="jasa_id"
                                    ...
                                    @foreach ($jasaList as $jasa)
                                        <option value="{{ $jasa->id }}"
                                            {{ old('jasa_id', $subJasa->jasa_id) == $jasa->id ? 'selected' : '' }}>
                                            {{ $jasa->nama }}
                                        </option>
                                    @endforeach
                                </select>
                                <p class="mt-1 text-xs text-gray-500 font-roboto">Pilih jasa utama untuk sub jasa ini</p>
                            </div>

                            <div class="sm:col-span-2">
                                <label for="nama" class="block mb-1 text-sm font-medium font-poppins">Nama Sub
                                    Jasa</label>
                                <input type="text" name="nama" id="nama"
                                    value="{{ old('nama', $subJasa->nama) }}"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-500 font-roboto"
                                    required>
                            </div>

                            <div class="sm:col-span-2">
                                <label for="harga" class="block mb-1 text-sm font-medium font-poppins">Harga (Rp)</label>
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <span class="text-gray-500 sm:text-sm font-roboto">Rp</span>
                                    </div>
                                    <input type="number" name="harga" id="harga"
                                        value="{{ old('harga', $subJasa->harga) }}"
                                        class="w-full pl-10 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-500 font-roboto"
                                        placeholder="0" min="0" required>
                                </div>
                                <p class="mt-1 text-xs text-gray-500 font-roboto">Masukkan harga tanpa tanda titik atau koma
                                </p>
                            </div>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="deskripsi" id="deskripsi" rows="4"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-500 font-roboto">{{ old('deskripsi', $subJasa->deskripsi) }}</textarea>
                                <p class="mt-1 text-xs text-gray-500 font-roboto">Jelaskan detail sub jasa yang ditawarkan
                                </p>
                            </div>
                            ...
                            <div class="sm:col-span-2">
                                    ...
                                    <span class="inline-block h-40 w-64 overflow-hidden bg-gray-100 rounded-lg"
                                        ...
                                        @if ($subJasa->gambar)
                                            <img src="{{ Storage::url($subJasa->gambar) }}" alt="{{ $subJasa->nama }}"
                                                class="h-full w-full object-cover" id="current_image">
                                        @else
                                ...
                                <p class="mt-2 text-xs text-gray-500 font-roboto">Format: JPG, PNG. Maks: 2MB. Kosongkan
            ...
            imageInput.addEventListener('change', function(event) {
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        // Tampilkan preview dan sembunyikan gambar lainnya
                    ...
                    // Jika tidak ada file, kembalikan ke gambar yang sudah ada atau default
